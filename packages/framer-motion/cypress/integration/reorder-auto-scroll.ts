...
describe("Reorder auto-scroll", () => {
    ...
    describe("with page-level scrolling", () => {
        it("Auto-scrolls down when dragging near bottom of viewport", () => {
            cy.visit("?test=reorder-auto-scroll-page")
                ...
                .then(() => {
                    cy.window().then((win) => {
                        ...
                        const nearBottom = viewportHeight - 20
                        ...
                        cy.get("[data-testid='0']")
                            ...
                            .window()
                            .then((w) => {
                                expect(w.scrollY).to.be.greaterThan(0)
                            })
                            .get("[data-testid='0']")
                            .trigger("pointerup", { force: true })
                    })
        ...
        it("Auto-scrolls up when dragging near top of viewport", () => {
            cy.visit("?test=reorder-auto-scroll-page")
                .wait(200)
                .window()
                .then((win) => {
                ...
                .window()
                .should((win) => {
                    expect(win.scrollY).to.equal(200)
                })
                .get("[data-testid='8']")
                .trigger("pointerdown", 50, 25)
                .wait(50)
                .trigger("pointermove", 50, 20, { force: true })
                .wait(50)
                .trigger("pointermove", 50, 10, { force: true })
                .wait(300)
                .window()
                .then((win) => {
                    expect(win.scrollY).to.be.lessThan(200)
                })
                .get("[data-testid='8']")
                .trigger("pointerup", { force: true })
        })
